...
class GateAction:
    """
    Populate a Gate Action
    """

    def __init__(self, input_flows: list[str], output_flow_label: str, activity: 'Activity'):
        """
        Collect the data necessary to populate a Gate Action

        Args:
            input_flows: The id of two or more input_flows
        ...
        if len(input_flows) < 2:
            msg = f"Gate requires at least two input flow ids, but got: {input_flows} at {self.activity.activity_path}"
            _logger.error(msg)
            raise ActionException(msg)

        # Get Flow_ap for each fid
        self.input_flows = [Flow.lookup_data(fid=f, anum=self.anum, domain=self.domain) for f in input_flows]

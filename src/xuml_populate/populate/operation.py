...
class Operation:
    ...
    @classmethod
    def populate(cls, mmdb: 'Tk', subsys_name: str, domain_name: str, parsed_op, first_op: bool):
        """
        ...
        signum = Signature.populate(mmdb, subsys=subsys_name, domain=domain_name)
        Relvar.insert(relvar='Operation_Signature', tuples=[
            Operation_Signature_i(SIGnum=signum, Operation=parsed_op.op, EE=parsed_op.ee, Domain=domain_name)
        ])

        Relvar.insert(relvar='Operation', tuples=[
            Operation_i(Name=parsed_op.op, EE=parsed_op.ee, Domain=domain_name, Direction=parsed_op.op_type)
        ])

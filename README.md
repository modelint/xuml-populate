...
<p align="center">
  <img src="https://raw.githubusercontent.com/modelint/xuml-populate/main/docs/images/popsystem.png"
       alt="popsystem populates your Executable UML system package into a Shlaer-Mellor metamodel repository, using the empty metamodel supplied by the installed xuml-metamodel package"
       width="720">
</p>
...
The empty metamodel repository (`mmdb.ral`) is supplied automatically by the
[`xuml-metamodel`](https://pypi.org/project/xuml-metamodel/) package, which `popsystem` depends on — so you do
not provide it yourself. That package is generated by the
[makexumlrepo command](https://github.com/modelint/make-xuml-repo) and versioned independently; to populate
...
By default, `popsystem` parses and populates the action language (Scrall) along with the model structure (classes,
...
(`xcm-parser`, `xsm-parser`, `mtd-parser`, `op-parser`, `scrall`, `mi-pyral`, `pyyaml`). It also pulls in
[`xuml-metamodel`](https://pypi.org/project/xuml-metamodel/), which provides the empty metamodel schema
that `popsystem` populates — so there is nothing extra to download or place in your working directory.
